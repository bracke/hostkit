...
procedure Check_All_Selftest is
   ...
   Bad_Root   : constant String := "/tmp/hostkit-check-all-selftest";
   Bad_Status : Integer;
begin
   ...
   Put_Line ("hostkit aggregate checker self-test passed");
   Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Success);
exception
   when E : others =>
      Put_Line
        (Standard_Error,
         "hostkit aggregate checker self-test failed: " & Ada.Exceptions.Exception_Message (E));
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
