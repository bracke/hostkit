...
package body Hostkit.Terminal_Control is
   ...
   --  TIOCGWINSZ and TIOCSWINSZ, macOS. BSD encodes an ioctl request from its
   --  direction, the size of the argument and a two-character group, so these
   --  are not the small numbers Linux uses (0x5413 and 0x5414) and cannot be
   --  shared with it. 0x40087468 is "read 8 bytes, group t, number 104".
   TIOCGWINSZ : constant Interfaces.C.unsigned_long := 16#4008_7468#;
   TIOCSWINSZ : constant Interfaces.C.unsigned_long := 16#8008_7467#;
   ...
   --  struct winsize. Four unsigned shorts on both macOS and Linux; this is one
