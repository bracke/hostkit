...
package body Hostkit.Spawn is
   ...
   --  TIOCSCTTY, macOS: "make this terminal my controlling terminal". BSD
   --  encodes an ioctl request from its direction, the size of the argument and
   --  a two-character group, so this is not the small number Linux uses
   --  (0x540E). 0x20007461 is "no argument, group t, number 97".
   TIOCSCTTY : constant Interfaces.C.unsigned_long := 16#2000_7461#;
