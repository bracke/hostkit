...
package body Hostkit.Descriptors is
   ...
   --  Linux, asm-generic/fcntl.h. Written in octal as the header does, so a
   --  reader can compare them with it directly. These are NOT the macOS values
   --  -- O_CREAT there is 0x200 and O_TRUNC 0x400 -- which is why each host has
   --  its own body rather than a shared one with a table.
   O_Rdonly   : constant C_Int := 8#0#;
   O_Wronly   : constant C_Int := 8#1#;
   O_Rdwr     : constant C_Int := 8#2#;
   O_Creat    : constant C_Int := 8#100#;
   O_Excl     : constant C_Int := 8#200#;
   O_Trunc    : constant C_Int := 8#1000#;
   O_Append   : constant C_Int := 8#2000#;
   O_Nonblock : constant C_Int := 8#4000#;

   --  fcntl commands and the close-on-exec flag.
   ...
   --  errno values this package distinguishes. Everything else is an error.
   E_Intr       : constant C_Int := 4;
   E_Again      : constant C_Int := 11;
   E_Pipe       : constant C_Int := 32;
   E_Wouldblock : constant C_Int := E_Again;  --  same value on Linux
   ...
   --  errno is a macro over a per-thread location; __errno_location is the
   --  function it expands to on glibc. Reading a global named "errno" would
   --  link on some hosts and read the wrong thread's value on all of them.
